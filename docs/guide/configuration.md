...
| `publications.yaml` | 出版物列表（Cite 弹窗、Highwire meta 由此生成）；条目的 `thumbnail: assets/…` 是首页 `publication-list` 块左侧的展示图 | 你 |
