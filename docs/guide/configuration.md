...
- **`modules`** —— 站点有哪些部分。`blog` / `pages` / `publications` / `projects` / `cv` 五个开关；`false` 意味着路由、导航、打包一起消失（约束 5）。写成对象可覆盖落地页文案。
...
- **`theme`** —— 外观。`name` 选主题（内置 `scholar` 默认、`paper`，或 `extensions/themes/` 里装的），`accent` 只换主色，`tokens` 逐个覆盖设计变量，`options` 是该主题自己声明的选项。详见 [THEMING.md](../THEMING.md)。
- **`i18n`** —— 默认语言走根路径，其余带 `/zh` 一类前缀。
