...
> 前置：`PLANNING.md` §3.3、`ARCHITECTURE.md`、ADR-003。§0 是阶段 1 就要交付的 static 自动重建路径；§1 起是阶段 2 的 server 模式。不涉及页面与样式。
...
server 模式在此之上换取秒级发布与运行时功能，代价见 `PLANNING.md` §3.3 对比表；切换时内容目录与模板不变。
