...
- [x] 主题契约 v2（ADR-032，2026-09-11）：部件注册表覆盖首页各节 + 出版物行/文章行/站点头尾；69 个 `data-part` 挂钩；density/labels 预设进 CSS；`pnpm theme:check`；示例主题 gutter（方案 C，零 src；随模板放在 extensions/themes/gutter）与 paper 在 CI 固定跑全套 e2e
