...
| 类型 | 名称 | 何时需要 |
| --- | --- | --- |
| Variable | `SITE_URL` | 总是（canonical / sitemap / feed 用的公网地址） |
...
核心功能全部完成并经双模式一致性 e2e、无障碍（axe）与 Lighthouse 门槛验收；正在做开源发布的收尾。任务清单见 [docs/dev/ROADMAP.md](docs/dev/ROADMAP.md)。已知限制：站内链接按域名根路径生成，尚不支持 `user.github.io/repo` 这类子路径部署（见部署文档的解法）。
