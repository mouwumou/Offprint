...
ADR-001–031 已定（ADR-013 为方向已定、细节待敲定：lang/urlname 由 sync 派生，Notion 不加列，LLM 翻译管线另行设计）；配置在根目录 `site.yaml`（ADR-021，i18n.default = en，不含任何内容），作者信息在 `content/profile.yaml`（ADR-028，经 `getProvider().getProfile()` 取），首页排布在 `content/home.yaml`，改 zod 配置 schema 后须 `pnpm gen:schema` 再生编辑器补全用的 JSON Schema。
