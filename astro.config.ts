...
// static is the default and baseline; server is an optional runtime.
const runtimeMode = process.env.RUNTIME_MODE === 'server' ? 'server' : 'static'

// SITE_URL is the single source for both the origin (`site`) and
// the deployment sub-path (`base`, e.g. https://user.github.io/repo → /repo).
const siteUrl = process.env.SITE_URL ?? 'https://example.com'
const base = basePathFromSiteUrl(siteUrl)

// site.yaml `redirects` → Astro redirects: meta-refresh pages on
// static hosts, real 30x responses under the node adapter. Astro
...
export default defineConfig({
  ...
  build: {
    ...
    // is what keeps mobile-lab LCP under the ≥0.95 Lighthouse bar.
    inlineStylesheets: 'always',
  },
  integrations: [
    react(),
    // Injects the server-only /api routes when RUNTIME_MODE=server;
    // a static build never bundles them (server-only code never enters a static build).
    offprint(),
  ...
  vite: {
    ...
    // the server-only branches are dead code there (server-only code never enters a static build).
